...
public class SnowyPineHedgeBlock extends HedgeBlock {
    public static final MapCodec<SnowyPineHedgeBlock> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    .apply(instance, SnowyPineHedgeBlock::new)
    );
    protected final ParticleEffect leafParticleEffect;

    public SnowyPineHedgeBlock(float leafParticleChance, ParticleEffect leafParticleEffect, Settings settings) {
    ...
    public static BlockState getMeltedState(BlockState state, WorldAccess world, BlockPos pos) {
        return ModBlocks.PINE_HEDGE.getDefaultState().with(WATERLOGGED, state.get(WATERLOGGED));
    }
    ...
    @Override
    public MapCodec<SnowyPineHedgeBlock> getCodec() {
        return CODEC;
    }
}
