...
public abstract class HedgeBlock extends Block implements Waterloggable {
    ...
    public HedgeBlock(float leafParticleChance, Settings settings) {
        super(settings);
    ...
    private boolean shouldConnectTo(BlockState state, boolean faceFullSquare, Direction side) {
        ...
        return state.isIn(ModTags.Blocks.HEDGES) || !cannotConnect(state) && faceFullSquare || block instanceof LeavesBlock || bl;
    }
    ...
    private boolean shouldHavePost(BlockState state, BlockState aboveState, VoxelShape aboveShape) {
        boolean bl = aboveState.getBlock() instanceof HedgeBlock && (Boolean)aboveState.get(UP);
        if (bl) {
            return true;
        } else {
